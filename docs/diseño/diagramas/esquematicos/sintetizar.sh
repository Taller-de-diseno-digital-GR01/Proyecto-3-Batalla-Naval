...
yosys -q -p "read_json pre.json; ${EXTRA:+read_verilog -sv $EXTRA;} hierarchy -top $top; submod; setattr -set keep 1 w:[a-zA-Z]*; opt; wreduce; opt; techmap; opt -fast; dfflegalize -cell \$_DFF_P_ x; abc -g AND,OR,XOR,MUX; opt_clean; write_json $top.all.json" 2>&1 | { grep -v combinational || true; }
